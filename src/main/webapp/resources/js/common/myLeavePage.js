    ...
    success: function (result) {
      for (const leaveTotalDays of result) {
        if (leaveTotalDays && leaveTotalDays.empNo !== null) {
          var text =
            "<i class='fi fi-rr-calendar-check'></i>&nbsp;&nbsp;휴가 누적 사용일수&nbsp;&nbsp;<b>" +
            leaveTotalDays.totalDays +
            "일</b>&nbsp;&nbsp;&nbsp;&nbsp; " +
            "<i class='fi fi-rr-calendar'></i>&nbsp;&nbsp;남은 휴가일수&nbsp;&nbsp;<b>" +
            (12 - leaveTotalDays.totalDays) +
            "일</b>&nbsp;&nbsp;&nbsp;&nbsp;";
        } else {
          var text =
            "<i class='fi fi-rr-calendar-check'></i>&nbsp;&nbsp;휴가 누적 사용일수 : <b>0일</b>&nbsp;&nbsp;&nbsp;&nbsp; " +
  ...
  if (result.list && result.list.length > 0) {
    for (const row of result.list) {
      const tr =
        ...
        row.status +
        "</td>" +
        "<td><a href='/myLeaveView?leaveId=" +
        row.leaveId +
        "' class='" +
        (row.status === "승인" || row.status === "반려" ? "disable" : "btnO") +
        "'>수정</a></td>" +
        "<td><a href='/myLeaveDelete?leaveId=" +
        row.leaveId +
        "' class='" +
        (row.status === "승인" || row.status === "반려" ? "disable" : "btnX") +
        "'>삭제</a></td>" +
        "</tr>";
      $table.append(tr);
    }
