...
<body>
	...
	function addRow() { // 열 추가 함수
        ...
        for(var i = 0; i < 10; i++) { // 열에 데이터 추가  4 : 직무, 5: 부서, 6 : 고용일 나머지 : 텍스트 데이터 
			if(i == 0) $("#result tr").eq(-1).append('<td><input list="List'+ (++count) +'" class="productName" placeholder="검색 또는 선택" /><datalist id="List'+ count +'"><c:forEach items="${productNameList}" var="product"><option value="${product.productName}" data-product_code ="${product.productCode}" data-product_color="${product.productColor}" data-product_price="${product.productPrice}" data-product_cost="${product.productCost}" data-product_category="${product.productCategory}" ></option></c:forEach></datalist></td>');
