...
- **AJAX 페이징과 상태 보전**(오재덕): 페이지 전환 시 수정값 유실 → 임시 저장 후 재표시(향후 더 나은 상태관리 필요)
- **휴가/사원 데이터 분리 조회**(이환희): 데이터 부재 케이스로 오류 → 요청 분리·중첩처리로 해결, 제약조건을 사전 기획 단계에서 충분히 검토할 필요성 확인
- **차트 가독성/무한 새로고침**(박지연): 카테고리·월 필터 추가, 생성/조회 함수 분리 및 기본값 처리로 안정성 확보
